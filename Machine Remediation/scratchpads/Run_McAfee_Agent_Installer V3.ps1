...
$work_dir = 'C:\Temp\PROJECTS\Workstation remediation\08-21-20 Tanium\'
$input_file = "$work_dir"+"XXX"+".txt"                                   # Manually edit input file name.
...
# ============================================================================================
        # Trying to put a test condition here where failure writes to the fail log.
        # Checking that MASC service is running. 
        Write-Host "Checking $Computer for running agent service MASVC"
        if ($MASVC = Invoke-Command -ComputerName $_ {(GWMI Win32_Service -Filter "Name='MASVC'").state -eq 'Running'}) {
            Write-Host "$Computer agent running is: $MASVC"
        ...
        # I might want to use something like this as the test:   if (Test-WSMan $Computer -ErrorAction SilentlyContinue) {$testWinRM = $true} else {$testWinRM = $false}
...
# ============================================================================================
    ...
    ELSE {
        Write-Host "Installation FAILED on $Computer" -F Red
        $Computer | Out-File -filepath "$error_file" -Append      
        # Fail beep, downward tones let you know an iteration failed.
        [console]::beep(1000,300)
        [console]::beep(400,300) 
